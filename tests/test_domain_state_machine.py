...
def test_state_machine_accepts_valid_path() -> None:
    ...
    apply_transition(message, MessageState.QUEUED, now=datetime.now(UTC))
    apply_transition(message, MessageState.SENDING, now=datetime.now(UTC))
    apply_transition(message, MessageState.SENT_TO_CHANNEL, now=datetime.now(UTC))
...
def test_state_machine_rejects_invalid_transition() -> None:
    ...
    with pytest.raises(StateTransitionError):
        apply_transition(message, MessageState.ACKNOWLEDGED, now=datetime.now(UTC))
